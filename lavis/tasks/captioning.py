...
def coco_caption_eval(coco_gt_root, results_file, split, annotation_file=None, img_ids=[]):
    ...
    img_ids = []
    with open(annotation_file[split], 'r', encoding='utf-8') as file:
        data = json.load(file)
    ...
    with open('.cache/results_file_temp.json', 'w', encoding='utf-8') as file:
        json.dump(data, file, ensure_ascii=False, indent=4)

    coco = COCO(annotation_file[split])
    coco_result = coco.loadRes('.cache/results_file_temp.json')
