...
class DistributedWeightedSampler(Sampler):
    def __init__(self, dataset, weights, num_samples, replacement=True, num_replicas=None, rank=None):
        ...
        # Ensure the dataset is evenly divided across GPUs
        self.num_samples_per_replica = int(self.num_samples // self.num_replicas)

    def __iter__(self):
        # Get subset of weights for current replica
        g = torch.Generator()
        g.manual_seed(torch.initial_seed())
        indices = torch.multinomial(
            self.weights, self.num_samples_per_replica, self.replacement, generator=g
        ).tolist()
        return iter(indices)
...
class RunnerBase:
    ...
    def create_loaders(
        ...
        def _create_loader_weighted_sampler(dataset, num_workers, bsz, is_train, collate_fn):
            # create a single dataloader for each split
            if isinstance(dataset, ChainDataset) or isinstance(
                dataset, wds.DataPipeline
            ):
                ...
                if self.use_distributed:
                    sampler = DistributedWeightedSampler(
                        dataset=dataset, weights=dataset.datasets[0].weights, 
                        num_samples=len(dataset), num_replicas=get_world_size(),
                        rank=get_rank()
                    )
                    if not self.use_dist_eval_sampler:
                        # e.g. retrieval evaluation
        ...
        for dataset, bsz, is_train, collate_fn in zip(
            ...
            if isinstance(dataset, list) or isinstance(dataset, tuple):
                ...
                if is_train == False:
                    ...
                    loader = _create_loader_weighted_sampler(dataset, num_workers, bsz, is_train, collate_fn)
